...
/**
 * Конфигурация обработчика эмиссионной фазы.
 ...
 */
data class EmissionPhaseConfig(
    val name: String = "em_phase",
 ...
 */
class EmissionPhaseContext(
    val g: Generic,
    val selector: NeuronSelectorPorts,
    val runStep: hw_var,
    val outQueue: FifoOutIF
)

/**
 * Обработчик эмиссионной фазы. Он:
 ...
 * 4. Реализует emit через запись в выходную очередь.
 */
class EmissionPhaseUnit(private val instName: String = "em_phase") {

    fun emit(
        g: Generic,
        cfg: EmissionPhaseConfig,
        runtime: EmissionPhaseRuntime,
        selector: NeuronSelectorPorts,
        outQueue: FifoOutIF,
        customLogic: ((EmissionPhaseContext) -> Unit)? = null,
        irLogic: EmissionPhaseIrLogic? = null,
        ...
        // Сброс сигналов очереди по умолчанию
        outQueue.we_i.assign(0)
        outQueue.wr_data_i.assign(0)
     ...
     * когда runStep=1. Emit отображается на запись в выходную очередь.
     */
    private fun buildIrLogic(
        g: Generic,
        irLogic: EmissionPhaseIrLogic,
        ...
        fun emitBlock(block: IrBlock) {
            block.statements.forEach { stmt ->
                when (stmt) {
                    ...
                    is IrLoop -> error("IrLoop is not supported in emission phase auto-logic yet")

                    is ir.IrEmit -> {
                        // Записываем текущий индекс нейрона в очередь, если она не полна
                        g.begif(g.bnot(outQueue.full_o)); run {
                            outQueue.we_i.assign(1)
                            // Опционально помечаем целевой символ фактом эмиссии
                            stmt.target?.let { target ->
                                storage[target.name]?.assign(1)
                            }
                        }; g.endif()
                    }
                }
        ...
        return { ctx ->
            ...
            val emitPayload = irLogic.emitData?.invoke(ctx) ?: ctx.selector.postIndex
            outQueue.wr_data_i.assign(emitPayload)
    ...
    private fun applyOpcode(g: Generic, opcode: OpCode, operands: List<hw_param>): hw_param {
        ...
        return when (opcode) {
            ...
            OpCode.POW -> error("POW opcode is not supported in emission phase auto-logic")
        }
    }
    ...
    private fun resolveCondition(
        ...
        return when (condition.comparison) {
            ...
            ComparisonOp.LT -> error("LT is not supported in emission phase auto-logic yet")
            ComparisonOp.LTE -> error("LTE is not supported in emission phase auto-logic yet")
            ComparisonOp.GT -> error("GT is not supported in emission phase auto-logic yet")
            ComparisonOp.GTE -> error("GTE is not supported in emission phase auto-logic yet")
        }
    }
 ...
 * emitData позволяет переопределить полезную нагрузку для emit;
 * по умолчанию используется индекс постсинаптического нейрона.
 */
data class EmissionPhaseIrLogic(
    val block: IrBlock,
    val symbols: SymbolTable,
    val emitData: ((EmissionPhaseContext) -> hw_var)? = null
)
