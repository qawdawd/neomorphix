...
/**
 * Конфигурация обработчика соматической фазы.
 ...
 */
data class SomaticPhaseConfig(
    val name: String = "som_phase",
...
/**
 * Обработчик соматической фазы. Он:
 ...
 */
class SomaticPhaseUnit(private val instName: String = "som_phase") {

    fun emit(
        g: Generic,
        cfg: SomaticPhaseConfig,
        runtime: SomaticPhaseRuntime,
        selector: NeuronSelectorPorts,
        customLogic: ((SomaticPhaseContext) -> Unit)? = null,
        irLogic: SomaticPhaseIrLogic? = null,
        ...
        fun emitBlock(block: IrBlock) {
            block.statements.forEach { stmt ->
                when (stmt) {
                    ...
                    is IrLoop -> error("IrLoop is not supported in somatic phase auto-logic yet")

                    is ir.IrEmit -> {
                        // Заглушка: реализация emit будет добавлена при интеграции с выходной очередью
                    }
                }
            }
        }
    ...
    private fun applyOpcode(g: Generic, opcode: OpCode, operands: List<hw_param>): hw_param {
        ...
        return when (opcode) {
            ...
            OpCode.POW -> error("POW opcode is not supported in somatic phase auto-logic")
        }
    }
    ...
    private fun resolveCondition(
        ...
        return when (condition.comparison) {
            ...
            ComparisonOp.LT -> error("LT is not supported in somatic phase auto-logic yet")
            ComparisonOp.LTE -> error("LTE is not supported in somatic phase auto-logic yet")
            ComparisonOp.GT -> error("GT is not supported in somatic phase auto-logic yet")
            ComparisonOp.GTE -> error("GTE is not supported in somatic phase auto-logic yet")
        }
    }
 ...
 */
data class SomaticPhaseIrLogic(
    val block: IrBlock,
